...
exports.create = async (req, res) => {
    try {
        ...
        const saveData = new ProjectModel({ title, description, status, users:[req.userId,...users], createdBy: req.userId });
        await saveData.save();
...
exports.getAllProjectsByUserId = async (req, res) => {
    try {
        const data = await ProjectModel.find({ users: { $in: req.userId } })
        .populate("users")
        .populate("createdBy")
        .exec()
        return res.status(STATUS_CODE.OK).json({
